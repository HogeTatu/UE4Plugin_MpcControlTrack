...
FMovieSceneMpcColorTrackInstance::FMovieSceneMpcColorTrackInstance( UMovieSceneMpcColorTrack& InColorTrack )
...
void FMovieSceneMpcColorTrackInstance::SaveState(const TArray<TWeakObjectPtr<UObject>>& RuntimeObjects, IMovieScenePlayer& Player, FMovieSceneSequenceInstance& SequenceInstance)
{
...
void FMovieSceneMpcColorTrackInstance::RestoreState(const TArray<TWeakObjectPtr<UObject>>& RuntimeObjects, IMovieScenePlayer& Player, FMovieSceneSequenceInstance& SequenceInstance)
{
	for (auto ObjectPtr : RuntimeObjects )
	{
		UObject* Object = ObjectPtr.Get();
		...
		if( ColorType == EColorType::Slate )
		{
			...
			if (ColorValue != nullptr)
			{
				...
				TrySetMpcVectorValue(Object, ColorValue->GetSpecifiedColor());
			}
		}
		else if( ColorType == EColorType::Linear || ColorType == EColorType::Color )
		{
			...
			if (ColorValue != nullptr)
			{
				...
				TrySetMpcVectorValue(Object, *ColorValue);
			}
		}
		else
		{
...
void FMovieSceneMpcColorTrackInstance::Update(EMovieSceneUpdateData& UpdateData, const TArray<TWeakObjectPtr<UObject>>& RuntimeObjects, class IMovieScenePlayer& Player, FMovieSceneSequenceInstance& SequenceInstance)
{
	for (auto ObjectPtr : RuntimeObjects)
	{
		UObject* Object = ObjectPtr.Get();
		if ( Object != nullptr )
		{
			if ( ColorType == EColorType::Slate )
			{
				...
				if ( ColorTrack->Eval( UpdateData.Position, UpdateData.LastPosition, LinearColor ) )
				{
					...
					TrySetMpcVectorValue(Object, LinearColor);
				}
			}
			else if ( ColorType == EColorType::Color )
			{
				...
				if ( ColorTrack->Eval( UpdateData.Position, UpdateData.LastPosition, LinearColorValue ) )
				{
					...
					else
					{
						...
						TrySetMpcVectorValue(Object, LinearColorValue);
					}
				}
			}
			else
...
void FMovieSceneMpcColorTrackInstance::TrySetMpcVectorValue(UObject* Object, const FLinearColor& ColorValue)
{
	...
	if (MpcOwner)
	{
		...
		if (Collection)
		{
			...
			Instance->SetVectorParameterValue(ColorTrack->GetPropertyName(), ColorValue);
		}
	}
}


void FMovieSceneMpcColorTrackInstance::RefreshInstance( const TArray<TWeakObjectPtr<UObject>>& RuntimeObjects, class IMovieScenePlayer& Player, FMovieSceneSequenceInstance& SequenceInstance )
{
